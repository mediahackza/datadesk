...
class query_handler {
    ...
    static function update_meta($table) {
        ...
        $table->find_meta_data();
        $query = "UPDATE " . self::$meta_table_name . " SET last_updated=CURRENT_TIMESTAMP, str_name='". $table->get_name()."', data_source='".$table->get_source()."', description='".Utils::check_quotes($table->get_description())."', row_count=".$table->row_count.", col_count=".$table->col_count.", headings='".Utils::check_quotes($table->get_heading_string())."' WHERE id=".$table->get_id().";";
